...
class Transaction(object):
    ...
    def run(self):
        ...
        post_req = urllib2.Request(url=URL_BASE + '/post_tweet',
            headers={'Content-Type': 'application/json'},
            data=json.dumps(
                {
                    "username": username,
                    "body": random_message(random.randint(20,140))
                }))

        start_timer = time.time()
        resp = urllib2.urlopen(post_req)
        resp.read()
        latency = time.time() - start_timer
        self.custom_timers['post_tweet'] = latency
        assert (resp.code == 200), 'Bad Response: HTTP %s' % resp.code
